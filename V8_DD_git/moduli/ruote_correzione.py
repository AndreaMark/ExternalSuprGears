...
def correzione_dentature(t_re, delta_iniziale, theta_0, x_pignone, x_ruota, rho_pignone, rho_ruota, r_pignone, r_ruota, DentiP, DentiR, modulo):
    ...
    while (
        ...
        rapporto_cos = np.cos(theta_0)/np.cos(theta_lavoro)
        ######  CARATTERISTICHE GEOMETRICHE RUOTE CORRETTE, slide parte 18
        # Raggi delle primitive di lavoro
        Rp_lavoro = (DentiP*modulo/2) * rapporto_cos #np.cos(theta_lavoro)
        Rr_lavoro = (DentiR*modulo/2) * rapporto_cos #np.cos(theta_lavoro)
        
        # Modulo di lavoro
        m_lavoro =  modulo * rapporto_cos 

        # Coefficienti eta per addendum e dedendum
        eta_p = (rapporto_cos  - 1) * DentiP
        eta_r = (rapporto_cos - 1) * DentiR
    ...
    return x_pignone_finale, x_ruota_finale, theta_lavoro, accesso_corretto, recesso_corretto, m_lavoro, Rpp_corretto, Rpr_corretto, y, y_prime, iter