...
def calcola_eps_alpha(d_t_p, d_t_r, d_p_p, d_p_r, theta_l, PB) :              # internet
    """
    ...
    Args: 
        ...
        d_p_p [mm]: diametro di primitiva del pignone.
        d_p_r [mm]: diametro di primitiva della ruota.
    ...
    """
    g_in = 0.5*(np.sqrt(d_t_r**2 - d_p_r**2) - d_p_r*np.tan(theta_l))
    g_out = 0.5*(np.sqrt(d_t_p**2 - d_p_p**2) - d_p_p*np.tan(theta_l))
    eps_alpha = (g_in + g_out)/PB
    return eps_alpha

# fattore ausiliario per ZB
def calcola_M1(d_t_p, d_t_r, d_pp_p, d_pp_r, eps_alpha, z, z_prime, alpha_wt):# eq 17
    """
    ...
    Args: 
        ...
        eps_alpha : rapporto di condotta d'evolvente(trasversale)
