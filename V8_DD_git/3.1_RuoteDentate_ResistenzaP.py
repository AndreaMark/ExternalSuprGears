...
out_file = open("ResisenzaP.txt", "w")                     # scrivo il file log
...
v = vel_rot_s*raggio_corretto_pignone               # [m/s] velocità lineare al diametro primitivo/di lavoro
...
print("velocità di risonanza:", resonance_running_speed)                 
...
#PB = np.pi*m_corr*np.cos(theta_p)

#eps_alpha = rfp.calcola_eps_alpha(diametro_t_p, diametro_t_r, diametro_l_p, diametro_l_r, theta_l, PB)
...
E_factor, G_factor, H_factor = rff.calcola_EGH(rho_fp, u, modulo, x_p, theta_p, spr, DentiP)

theta_aus, iterazioni = rff.calcola_theta_aus(0.001, np.pi/6, DentiP, H_factor, G_factor)

####    tooth normal cord: spessore critico
sfn, sfn_norm = rff.calcola_sFn(G_factor, rho_fp, modulo, theta_aus, DentiP)

####    radius of root fillet, ρF: raggio di curvatura della sez. critica
rho_F = rff.calcola_rhoF(modulo, DentiP, theta_aus, G_factor, rho_fp)
...
alpha_en, den = rff.calcola_alphaen_den(dan, dbn, dn, theta_p, beta, DentiP, eps_alpha)
#### Tooth thickness half angle
gamma_e = rff.calcola_gamma_e(x_p, DentiP, theta_p, alpha_en)
#### Load direction angle,
alpha_fen = rff.calcola_alphaFen(alpha_en, gamma_e)                              # eq 31
hFE, hFE_norm = rff.calcola_hFe((gamma_e), (alpha_fen), den, modulo, (theta_aus), DentiP, G_factor, rho_fp)
...
d_alberomax = diametro_pp_p-(2*sr)
...
conn = sqlite3.connect("influence_factorsP.db")

# Salva il DataFrame nel database
IF.to_sql('influence_factorsP', conn, if_exists='replace', index_label='index')

# Chiudi la connessione
conn.close()

# Tabella su excel sempre facilmente consultabile
IF.to_excel('influence_factorsP.xlsx', index=True)
...
# Connessione al database SQLite
conn = sqlite3.connect("pitting_factorsP.db")

# Salva il DataFrame nel database
PF.to_sql('pitting_factorsP', conn, if_exists='replace', index_label='index')

# Chiudi la connessione
conn.close()

# Tabella su excel sempre facilmente consultabile
PF.to_excel('pitting_factorsP.xlsx', index=True)
...
# Connessione al database SQLite
conn = sqlite3.connect("bending_factorsP.db")

# Salva il DataFrame nel database
BF.to_sql('bending_factorsP', conn, if_exists='replace', index_label='index')

# Chiudi la connessione
conn.close()

# Tabella su excel sempre facilmente consultabile
BF.to_excel('bending_factorsP.xlsx', index=True)
