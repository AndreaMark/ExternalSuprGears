...
vel_rot_s = (VC.loc['r1','pignone'])                                                # [1/s] velocità di rotazione
...
theta_l = (PC.loc['r2','pignone'])                                              # [rad] angolo di lavoro                                                                 # [rad] angolo d'elica
m_corr = (PC.loc['r4','pignone'])                                               # [mm] modulo corretto                                                # [mm] modulo unificato
...
F_t = (PC.loc['r18','pignone'])                                 # [N] forza tangenziale                         
F_r = (PC.loc['r19','pignone'])                                 # [N] forza radiale
...
G_factor = rff.calcola_G(modulo, u, rho_fp, x_p)

H_factor = rff.calcola_H(modulo, E_factor, DentiP)

theta_aus, iterazioni = rff.calcola_theta_aus(0.01, np.pi/6, DentiP, H_factor, G_factor)

####    tooth normal cord
sfn, sfn_norm = rff.calcola_sfn(modulo, DentiP, theta_aus, G_factor, rho_fp)
...
####    Diametro primitivo
dn = m_uni*DentiP                                           # eq 24                                            
dbn = dn*np.cos(theta_l)                                    # eq 26
dan = dn + diametro_t_p - diametro_p_p                      # eq ??

alpha_en, den = rff.calcola_alphaen(dan, dbn, diametro_p_p, beta, theta_l, DentiP, eps_gamma)

gamma_e = rff.calcola_gamma_e(x_p, DentiP, theta_p, alpha_en)

alpha_fen = alpha_en - gamma_e                              # eq 31

hFE, hFE_norm = rff.calcola_hFE(DentiP, modulo, den, rho_fp, theta_aus, gamma_e, alpha_fen, G_factor)
...
IF.to_csv('influence_factorsP.csv', index=True)
...
PF.to_csv('pitting_factorsP.csv', index=True)
...
BF.to_csv('bending_factorsP.csv', index=True)
